...
class idebFinalYearsExtractor(AbstractDataExtractor):
    ...
    EXTRACTED_DATA_VAL_COL = "valor"

    __SCRAPPER_CLASS: IdebFinalYearsScrapper

    def __init__(self):
        self.__SCRAPPER_CLASS = IdebFinalYearsScrapper()

    def __treat_vals_and_add_cols(self, df: pd.DataFrame) -> pd.DataFrame:
        new_df = df.copy()

        # Substituir '--' e '-' por None, depois remover NaN
        new_df[self.EXTRACTED_DATA_VAL_COL] = new_df[self.EXTRACTED_DATA_VAL_COL].replace({"-": None, "--": None})
        new_df = new_df.dropna(axis="index")

        new_df[self.EXTRACTED_DATA_VAL_COL] = new_df[self.EXTRACTED_DATA_VAL_COL].astype("float")
        ...
        new_df = new_df.rename({
            ...
            self.EXTRACTED_DATA_VAL_COL: self.DATA_VALUE_COLUMN,
        }, axis="columns")
